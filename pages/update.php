...
$pid = intval($_GET['nid']);
$ret = mysqli_query($con, "select Lke from tblposts where id='$pid'");
$row = mysqli_fetch_assoc($ret);
$nlk = $row['Lke'];
$lk = $nlk + 1;
$query = mysqli_query($con, "update tblposts  set Lke='$lk' where tblposts.id='$pid'");
$query = mysqli_query($con, "select Lke,Lov,Unlke from tblposts where tblposts.id='$pid'");
while ($us = mysqli_fetch_array($query)) {
?>
    <i class="fa fa-thumbs-up" id="lke" style="font-size: 24px; color:#0066ff;"><span> <?php echo htmlentities($us['Lke']); ?></span></i>
    <i class="fa fa-heart" style="font-size: 24px;"><span> <?php echo htmlentities($us['Lov']); ?></span></i>
    <i class="fa fa-thumbs-down" style="font-size: 24px;"><span> <?php echo htmlentities($us['Unlke']); ?></span></i>
